...
public class BackgroundPlayService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        showPlayButton = intent.getStringExtra(KeyUtil.showPlayButton);
    ...
    private void startPlay() {
        ...
        FloatController floatController=  new FloatController(getApplicationContext());
        floatController.setPlayProgressButtonGone(showPlayButton);
        videoView.skipPositionWhenPlay(url, position).setVideoController(floatController).start();
    ...
    private void initWindow() {
        ...
        // 设置悬浮窗口长宽数据
        int width = WindowUtil.dp2px(getApplicationContext(), 110);
        wmParams.width = width;
        wmParams.height = width * 16 / 10;
